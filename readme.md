...
- 带权交叉熵：对档位进行分类，![](http://latex.codecogs.com/svg.latex?\sum_i^n\sum_j^5 w_j y_{ij}logp_{ij})，n个样本，![](http://latex.codecogs.com/svg.latex?y_{ij})代表第i个样本第j类的真实值，$p_{ij}$代表第i个样本第j类的预测值。$y_{ij}\in \{0,1\}$，$w_j$为该类的权重，$w_j \in \{1,10,50,100,300\}$
